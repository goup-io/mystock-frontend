            ...
            <div key={props.key} className="w-full bg-[#355070] rounded-md flex text-[#ffffff] shadow-[5px_5px_10px_0_rgba(0,0,0,0.14)] text-left drop-shadow-md">
                ...
                <div className="w-full p-2 px-4 flex flex-col">
                    ...
                    <div className="flex justify-end">
                        <a onClick={() => navigate(props.isTransferencia ? rotaTelaTransferencia : rotaTelaEstoque)} className="text-xs cursor-pointer underline duration-150 ease-in-out hover:text-[#A6CEE3] flex items-center gap-1">{props.isTransferencia ? 'Ver transferências' : 'Ver estoque'} <img src={iconSeta} className='h-3' /></a>
                    </div>
                </div>
            </div>
        </>
